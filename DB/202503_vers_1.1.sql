...
DROP TRIGGER IF EXISTS `trig_insert_visiteur`;CREATE DEFINER=`mjcbolbec`@`localhost` TRIGGER `trig_insert_visiteur` AFTER INSERT ON `visiteur` FOR EACH ROW INSERT INTO estPresent (idVisiteur, idActivite, present, estPresent.date) SELECT NEW.id, activite.id, "0", CURRENT_DATE() FROM activite